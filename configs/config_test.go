...
func TestConfig_ValidateConfig(t *testing.T) {
	config := NewConfig("", "", nil, "test-jiraProject", "brand/testing-repo", false, 0)
	err := config.ValidateConfig()
	assert.Equal(t, errTokenMissing, err)

	config2 := NewConfig("123token", "", nil, "test-jiraProject", "brand/testing-repo", false, 0)
	err2 := config2.ValidateConfig()
	assert.Equal(t, errWebhookUrlMissing, err2)

	config3 := NewConfig("123token", "webhook-url", nil, "", "brand/testing-repo", false, 0)
	err3 := config3.ValidateConfig()
	assert.Equal(t, errJiraProjectMissing, err3)

	config4 := NewConfig("123token", "webhook-url", nil, "test-jiraProject", "", false, 0)
	err4 := config4.ValidateConfig()
	assert.Equal(t, errGithubRepoMissing, err4)

	configValid := NewConfig("123token", "webhook-url", nil, "test-jiraProject", "brand/testing-repo", false, 0)
